...
# Calculate PP load (g) each timestep
...
with Simulation("./BRC_DynamicStorm_0.38mgL.inp") as sim:
    ...
    # Tracking time for control actions every 15 minutes (1 min time step)
    _tempcount = 15
    ...
    for index,step in enumerate(sim):
        ...
        # Get phosphorus concentrations        
        Cin_dp = soil.pollut_quality['DP']
        inflow_dpC.append(Cin_dp)
        Cin_pp = soil.pollut_quality['PP']
        inflow_ppC.append(Cin_pp)
...
with Simulation("./BRC_DynamicStorm_0.38mgL.inp") as sim:
    ...
    for index,step in enumerate(sim):
        ...
        # Get phosphorus concentrations        
        Cin_dp = soil.pollut_quality['DP']
        inflow_dpA.append(Cin_dp)
        Cin_pp = soil.pollut_quality['PP']
        inflow_ppA.append(Cin_pp)

        # Get depth data
        pond_heightA.append(pond.depth)
        soil_heightA.append(soil.depth)
        ...
        if index <= 3900:
            ...
            if Qin >= 0.001:
                ...
                t = t + 1
                # Calculate new concentration
                Cnew_dp = (Cin_dp*np.exp((-k_dpA*L*A*E)/Qin))+\
                    (Ceq0_dp*np.exp(B1_dpA*t))*(1-(np.exp((-k_dpA*L*A*E)/Qin)))
                Cnew_pp = (Cin_pp*np.exp((-k_ppA*L*A*E)/Qin))+\
                    (Ceq0_pp*np.exp(B1_ppA*t))*(1-(np.exp((-k_ppA*L*A*E)/Qin)))
        ...
        infil_dpA.append(Cnew_dp)
        infil_ppA.append(Cnew_pp)
        drained_dpA.append(Cnew_dp)
        drained_ppA.append(Cnew_pp)
        exfil_dpA.append(Cnew_dp)
        exfil_ppA.append(Cnew_pp)
...
# Plot Concentration
fig, ax = plt.subplots(1,3)
ax[0].plot(infil_dp, color='#6CC6D1', linewidth=2, label="DP")
ax[0].plot(infil_pp, '--', color='#6CC6D1', linewidth=2, label="PP")
ax[0].set_ylabel("Baseline DP & PP Conc. (mg/L)")
ax[0].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[0].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[0].set_xlabel("Time (days)")
ax[0].legend()

ax[1].plot(infil_dpC, color='#695580', linewidth=2, label="DP")
ax[1].plot(infil_ppC, '--', color='#695580', linewidth=2, label="PP")
ax[1].set_ylabel("Autonomous DP & PP Conc. (mg/L)")
ax[1].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[1].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[1].set_xlabel("Time (days)")
ax[1].legend()

ax[2].plot(infil_dpA, color='#3B4D7A', linewidth=2, label="DP")
ax[2].plot(infil_ppA, '--', color='#3B4D7A', linewidth=2, label="PP")
ax[2].set_ylabel("Passive DP & PP Conc. (mg/L)")
ax[2].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[2].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[2].set_xlabel("Time (days)")
ax[2].legend()
...
#Plot phosphorus load
fig, ax = plt.subplots(1,3)
ax[0].plot(released_load, color='#6CC6D1', linewidth=2, label="baseline")
ax[0].set_ylabel("Baseline TP Load (g)")
ax[0].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[0].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[0].set_xlabel("Time (days)")
ax[0].legend()

ax[1].plot(released_loadC, color='#695580', linewidth=2, label="autonomous")
ax[1].set_ylabel("Autonomous TP Load (g)")
ax[1].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[1].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[1].set_xlabel("Time (days)")
ax[1].legend()

ax[2].plot(released_loadA, color='#3B4D7A', linewidth=2, label="passive")
ax[2].set_ylabel("Passive TP Load (g)")
ax[2].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[2].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[2].set_xlabel("Time (days)")
ax[2].legend()
...
#Plot mass flow rate
fig, ax = plt.subplots(1,3)
ax[0].plot(inflow_mfr_tpC, color='#695580', linewidth=2, label="autonomous")
ax[0].plot(inflow_mfr_tp, '-.', color='#6CC6D1', linewidth=2, label="baseline")
ax[0].plot(inflow_mfr_tpA, '--', color='#3B4D7A', linewidth=2, label="passive")
ax[0].set_ylabel("Influent mass flow rate (mg/s)")
ax[0].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[0].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[0].set_xlabel("Time (days)")
ax[0].legend()

ax[1].plot(drain_mfr_tpC, color='#695580', linewidth=2, label="autonomous")
ax[1].plot(drain_mfr_tp, '-.', color='#6CC6D1', linewidth=2, label="baseline")
ax[1].plot(drain_mfr_tpA, '--', color='#3B4D7A', linewidth=2, label="passive")
ax[1].set_ylabel("Released mass flow rate (mg/s)")
ax[1].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[1].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[1].set_xlabel("Time (days)")

ax[2].plot(exfil_mfr_tpC, color='#695580', linewidth=2, label="autonomous")
ax[2].plot(exfil_mfr_tp, '-.', color='#6CC6D1', linewidth=2, label="baseline")
ax[2].plot(exfil_mfr_tpA, '--', color='#3B4D7A', linewidth=2, label="passive")
ax[2].set_ylabel("Captured mass flow rate (mg/s)")
ax[2].set_xticks([0,1337,2674,4011,5348,6686,8023,9360])
ax[2].set_xticklabels(["0","1","2","3","4","5","6","7"])
ax[2].set_xlabel("Time (days)")
plt.show()
